...
loginctl enable-linger "$USER"                       # headless 开机自启（关键）
...
│   └── custom/                          ← @custom FDE 改这里
│       ├── agent-template.plist          ← launchd plist 模板（macOS）
│       └── agent-template.service        ← systemd user unit 模板（Linux）
│
├── tests/
...
- **跨平台守护（macOS + Linux）**：launchd（macOS）与 systemd user unit（Linux，如 Raspberry Pi）均支持，模板见 `bin/custom/agent-template.{plist,service}`。`bin/core/*.sh` 已按 `harness_os` 分派平台差异（supervisor 重启、`date`），reboot.sh 的 `SUPERVISOR_RESTART_CMD` 可覆盖为容器/system 级方式。Windows 仍需自己适配。
