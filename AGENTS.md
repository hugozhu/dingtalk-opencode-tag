...
- e2e 通用范式：**实际触发 + 监控日志 + DWS 独立拉群消息**（两侧对照的双校验）
  ...
  - **注意后端两条路不一样**：普通文本回复走 `brain._brain_opencode` 的 `opencode run` 一次性子进程（无需 serve）；只有合并转发业务走 `inject_and_forward` 的 opencode serve HTTP `/session`。"回复正常"不等于"serve HTTP 通"，两者要分开验。
