...
- e2e 通用范式：**实际触发 + 监控日志 + DWS 独立拉群消息**（两侧对照的双校验）
  ...
  - **ack 收尾 e2e（本地冒烟，无需真发）**：把 `ACK_PROGRESS_INTERVAL` 压到 1s，验证主管裁决后两条消息的 ack worker 都收尾、不再周期播「仍在处理中」（#108）。同样带 `E2E_SIMULATE_BUG=1` 反证。见 `tests/custom/e2e_supervisor_ack_test.py`。
