...
# **默认关**：文字表情依赖 DingTalk create-text-emotion，实测部分文案报"暂不支持保存该文字表情"
# （如默认的 "🈺 已收到，正在处理…"），且需数字员工 profile 有权限。确认你钉钉可用的表情名+文案
# （先用 `dws chat message create-text-emotion --emotion-name <名> --text <文>` 手测能否保存）+
# 权限后，在 config/constants.local.sh 设 CAP_ACK_ENABLED=1 开启。
export CAP_ACK_ENABLED="${CAP_ACK_ENABLED:-0}"
...
export ACK_STAGES="${ACK_STAGES:-0:稍等:🈺 已收到，正在处理…|5:稍等:⏳ 正在处理中…|300:咖啡:⏳ 仍在处理（约 5 分钟），请稍候…}"
export ACK_DONE="${ACK_DONE:-OK:✅ 已处理完成}"          # 完成（表情名:文字）
export ACK_ERROR="${ACK_ERROR:-疑问:⚠️ 处理未完成}"      # 失败（表情名:文字）
